...
router.openapi(batchRoute, async (c) => {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (dupCheck.length > 0) {
      results.push({ index: i, status: "duplicate", existing_slug: dupCheck[0].slug });
      failed++;
      continue;
    }
    ...
    try {
      ...
      if (simResult.rows.length > 0) {
        results.push({
          index: i,
          status: "similar",
          similar_submissions: (simResult.rows as Array<{ slug: string; delta: number | null; sim: number }>).map((r) => ({
            slug: r.slug,
            delta: Number(r.delta ?? 0),
            similarity: Number(r.sim),
          })),
    ...
    } catch { /* pg_trgm not installed — skip */ }
    ...
    if (fixForSlug) {
      const fixForCheck = await db
        ...
        .limit(1);
      if (fixForCheck.length === 0) {
        results.push({ index: i, status: "error", code: "NOT_FOUND", message: "The submission to fix does not exist or is not approved." });
        failed++;
        continue;
      }
    }
    ...
    if (supersededSlug) {
      ...
      if (prev.length === 0) {
        results.push({ index: i, status: "error", code: "NOT_FOUND", message: "The submission to supersede does not exist." });
        failed++;
        continue;
      }
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      try {
        ...
        if (attempt < 4 && isSlugConflict(e)) continue;
        results.push({ index: i, status: "error", code: "INTERNAL_ERROR", message: "Failed to insert submission." });
        failed++;
        break;
      }
    }
    ...
    if (insertOk) {
      ...
      results.push({ index: i, status: "created", slug, url: `/s/${slug}` });
      created++;
    }
  }
